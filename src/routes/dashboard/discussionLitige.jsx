...
export default function DiscussionLitige({ onClose }) {
        ...
        setMessages([
          {
            ...
            avatar:
              litiges?.avatar || "https://randomuser.me/api/portraits/men/32.jpg",
            content:
              litiges?.message || "Bonjour, j’ai un souci avec ma commande.",
    ...
    if (window.confirm("Confirmer que ce litige a été traité ?")) {
      ...
      setMessages([
        ...messages,
        {
          ...
          avatar: "/admin-avatar.png",
          content:
            "<span class='text-green-500 font-semibold'>✅ Ce litige a été traité.</span>",
          time: new Date().toISOString(),
        },
      ]);
    }
  ...
  const saveEdit = () => {
    setMessages(
      messages.map((m) => (m.id === editingId ? { ...m, content: editedContent } : m))
    );
    setEditingId(null);
    setEditedContent("");
  };
  ...
  if (!litiges)
    return <p className="text-center p-4 text-gray-500">Aucun litige sélectionné.</p>;
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100 overflow-x-hidden">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              <div
                className={`p-3 rounded-lg relative text-sm ${
                  msg.sender === "admin"
                ...
                {editingId === msg.id ? (
                  <>
                    <input
                      ...
                      className="text-black w-full mb-2 p-1 rounded"
                    />
                    <button
                      onClick={saveEdit}
                      className="text-xs text-green-300 mr-2"
                    >
                      Sauvegarder
                    </button>
                    <button
                  ...
                  <>
                    ...
                    <span className="text-xs block mt-1 text-gray-300">
                      {formatTime(msg.time)}
                    </span>
      ...
      {/* Champ de réponse */}
      <div className="p-4 border-t bg-white flex flex-wrap sm:flex-nowrap items-center gap-2 w-full max-w-full">
        <input
          value={newMessage}
          onChange={(e) => setNewMessage(e.target.value)}
          disabled={litigeTraite}
          placeholder={litigeTraite ? "Litige traité." : "Écrivez un message..."}
          className="flex-1 px-4 py-2 border rounded-full w-full"
        />
        <button
          onClick={handleSend}
          disabled={litigeTraite}
