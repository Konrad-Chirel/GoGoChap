...
export default function DiscussionLitige({onClose}) {
 ...
 useEffect(() => {
  ...
  // ✅ CORRIGÉ : Navigation retour améliorée pour mobile
  const handleRetour = () => {
    // Utilise navigate avec le chemin spécifique au lieu de navigate(-1)
  ...
  return (
    // ✅ CORRIGÉ : Remplacé h-screen par h-full pour éviter les conflits avec le layout parent
    <div className="flex flex-col h-full bg-gray-100 min-h-0">
      {/* En-tête */}
      <div className="bg-white shadow p-4 border-b flex justify-between items-start flex-shrink-0">
        <button 
      ...
      {/* ✅ CORRIGÉ : Messages avec flex-1 et min-h-0 pour une meilleure gestion de l'espace */}
      ...
      {/* ✅ CORRIGÉ : Champ de réponse avec flex-shrink-0 pour éviter qu'il soit compressé */}
      <div className="p-4 border-t bg-white flex items-center gap-2 flex-shrink-0">
        <input
          value={newMessage}
        ...
        <button
          onClick={handleSend}
          disabled={litigeTraite}
          className="bg-blue-600 text-white px-4 py-2 rounded-full disabled:opacity-50 flex-shrink-0"
        >
          Envoyer
        </button>
      </div>
